...
def target_k(source, nlp):
    source_ents = num_ents(source, nlp)
    if source_ents < 25:
        target_ents = 6
    elif source_ents < 100:
        target_ents = 10
    elif source_ents < 150:
        target_ents = 11
    elif source_ents < 175:
        target_ents = 12
    else:
        target_ents = 13

    return source_ents, target_ents


if __name__ == '__main__':
    parser = argparse.ArgumentParser('xxx')
    parser.add_argument('--experiment', default='human_straight_dense_v2')
    ...
    dense_prompt = open('straight_dense_prompt.txt').read().strip()
    ...
    for example in tqdm(dataset, total=len(dataset)):
        ...
        source_ents, target_ents = target_k(source, nlp)

        messages = [
            {'role': 'system', 'content': 'You are a space-efficient assistant for text summarization.'},
            {'role': 'user', 'content': dense_prompt.replace('{{ARTICLE}}', source).replace('{{K}}', str(target_ents))}
        ]

        output = json.loads(chatgpt(messages))

        example['prediction'] = output['summary']
        example['entities'] = output['entities']
        example['source_ents'] = source_ents
        example['target_ents'] = target_ents
