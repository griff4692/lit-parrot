...
ORACLE_STRATEGY = 'update'
DEBUG = False
debug_str = '_mini' if DEBUG else ''
IN_DIR = os.path.expanduser(f"~/note_partials{debug_str}_oracle_{ORACLE_STRATEGY}_llm")
DESTINATION_PATH = Path(os.path.expanduser(f"~/lit-parrot/data/bhc_{ORACLE_STRATEGY}"))
CHECKPOINT_DIR = Path(os.path.expanduser("~/lit-parrot/checkpoints/tiiuae/falcon-7b"))
IGNORE_INDEX = -1
MASK_INPUTS = True
SEED = 42
...
def prepare(
    ...
    print(f'Changing Max Sequence Length to from {max_seq_length} --> 3072')
    max_seq_length = 3072
    ...
    print(f"test has {len(test_set)}/{n_test} samples after removing too long.")
